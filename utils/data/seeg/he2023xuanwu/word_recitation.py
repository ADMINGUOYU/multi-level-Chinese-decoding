...
def _load_subj_bipolar(path_run_datasets, ch_names=None):
    ...
    def _pick_xn(sample):
        x = getattr(sample, "data_n", None)
        if x is None:
            x = getattr(sample, "data_s", None)
        if x is None:
            raise AttributeError("Neither `data_n` nor `data_s` found in sample.")
        return x
    ...
    for dataset_info_i, dataset_data_i in zip(dataset_info, dataset_data):
        ...
        # X_s 始终来自 data_s；X_n 优先 data_n，不存在则回退到 data_s
        X_s_i = np.array([
            (getattr(data_i, "data_s", getattr(data_i, "data_n", None))).T
            for data_i in dataset_data_i
        ], dtype=np.float32)[:, :, ch_idxs_i]
        X_n_i = np.array([
            (_pick_xn(data_i)).T
            for data_i in dataset_data_i
        ], dtype=np.float32)[:, :, ch_idxs_i]
        ...
        if len(labels) == 61:  # 只输出一次，避免打印太多
            print(f"[DEBUG] Subj {subj}: labels order (前10个) ->", labels[:10])
