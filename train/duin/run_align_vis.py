...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    emb_words = emb_data["chars"]  # 或 "chars"，看 npz 内部字段名
    print("Meta info:\n", emb_data["meta"])
...
def train():   # 修改训练循环中的损失函数、输出日志
    ...
    for load_params_idx in range(len(load_params)):
        ...
        msg = (
            "Finish the (Visual Embedding Alignment) training process of experiment {}."
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("DuIN Visual Alignment for brain signals", add_help=False)
...
if __name__ == "__main__":
    ...
    # Save the run_script path if provided
    duin_params_inst.train.run_script = args.run_script
