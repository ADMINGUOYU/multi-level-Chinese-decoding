...
class AlignHead(nn.Module):
    ...
    Projects encoder embeddings into target embedding space (e.g., CLIP or other supervision).
    """

    def __init__(self, params, **kwargs):
        """
        Initialize `AlignHead` object.

        Args:
            params: DotDict - The parameters of `AlignHead`.
            kwargs: dict - The arguments related to initialize `nn.Module`-style object.
        """
        super(AlignHead, self).__init__(**kwargs)
    ...
    def _init_model(self):
        ...
        # Input: (batch_size, emb_len, d_model)
        # Output: (batch_size, d_output)
        self.align_head = nn.Sequential()

        # Step 1. Flatten embeddings across sequence dimension.
        # Shape: (batch_size, emb_len * d_model)
        self.align_head.append(nn.Flatten(start_dim=1, end_dim=-1))

        # Step 2. Hidden Linear layers with ReLU activations.
        for hidden_idx in range(len(self.params.d_hidden)):
            self.align_head.append(nn.Sequential(
                nn.Linear(
                    in_features=(self.params.d_hidden[hidden_idx-1] if hidden_idx > 0 else self.params.d_feature),
                    out_features=self.params.d_hidden[hidden_idx],
                    bias=True, device=None, dtype=None
        ...
        # Step 3. Dropout layer for regularization.
        if self.params.dropout > 0.:
            self.align_head.append(nn.Dropout(p=self.params.dropout, inplace=False))

        # Step 4. Final Linear projection to target embedding dimension (e.g., 768).
        self.align_head.append(nn.Linear(
            in_features=(self.params.d_hidden[-1] if len(self.params.d_hidden) > 0 else self.params.d_feature),
            out_features=self.params.d_output,
            bias=True, device=None, dtype=None
        ))

        # ⚠️ No activation (Sigmoid/ReLU) after final layer — preserve embedding continuity.
    ...
    def forward(self, E):
        """
        Forward pass for alignment head.

        Args:
            E: (batch_size, emb_len, d_model) - Encoder-transformed embeddings.

        Returns:
            Z_pred: (batch_size, d_output) - Predicted embeddings aligned to supervision space.
        """
        return self.align_head(E)
